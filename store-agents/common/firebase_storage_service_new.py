    ...
    def _initialize_firebase(self):
        ...
        try:
                ...
                if project_id:
                    ...
                    for bucket_name in bucket_patterns:
                        try:
                            ...
                            # Test if bucket is accessible by trying to list blobs
                            try:
                                ...
                                logger.warning(f"Bucket {bucket_name} exists but not accessible: {list_error}")
                                continue
                        except Exception as e:
                    ...
                    if not bucket_connected:
                        ...
                        logger.warning("Firebase Storage will not be available - using local fallback")
                        self.bucket = None
                else:
                    ...
                    try:
                        ...
                        # Test accessibility
                        list(self.bucket.list_blobs(max_results=1))
                        logger.info("✅ Connected to default bucket")
                    except Exception as e:
                        logger.warning(f"Default bucket not accessible: {e}")
                        self.bucket = None
            ...
            if self.bucket:
                ...
                logger.warning("⚠️ Firebase Storage not available - will use local storage fallback")
                
        except Exception as e:
            logger.error(f"Failed to initialize Firebase Storage: {str(e)}")
            logger.warning("Will use local storage fallback")
            self.bucket = None
